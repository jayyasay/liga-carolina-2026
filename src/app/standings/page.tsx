  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '40px 20px' }}>
      ...
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px' }}>
        <div>
          <h1 className="brand-gradient" style={{ fontSize: '2rem' }}>LEAGUE STANDINGS</h1>
          <p style={{ color: 'var(--text-secondary)' }}>Live Leaderboard Generated from Match Outcomes</p>
        </div>
        <div style={{ display: 'flex', gap: '12px' }}>
          <Link href="/" className="secondary-btn" style={{ textDecoration: 'none' }}>Live Matches</Link>
      ...
      <div style={{ marginBottom: '40px', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
         <Link href="/standings" className="secondary-btn" style={{ padding: '6px 12px', fontSize: '0.9rem', textDecoration: 'none', background: !filterDivision ? 'rgba(0,0,0,0.4)' : 'var(--surface-hover)', borderColor: !filterDivision ? 'var(--brand-primary)' : 'transparent', color: !filterDivision ? 'var(--brand-primary)' : 'var(--text-secondary)' }}>
             Overall League
         </Link>
         {validDivisions.map(div => (
             <Link key={div} href={`/standings?division=${encodeURIComponent(div)}`} className="secondary-btn" style={{ padding: '6px 12px', fontSize: '0.9rem', textDecoration: 'none', background: filterDivision === div ? 'rgba(0,0,0,0.4)' : 'var(--surface-hover)', borderColor: filterDivision === div ? 'var(--brand-cyan)' : 'transparent', color: filterDivision === div ? 'var(--brand-cyan)' : 'var(--text-secondary)' }}>
                 {div}
             </Link>
         ))}
      </div>
          ...
          <table className="data-table" style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
              <thead style={{ background: 'var(--surface-hover)' }}>
                  <tr>
                      ...
                      <th style={{ padding: '16px' }}>Franchise</th>
